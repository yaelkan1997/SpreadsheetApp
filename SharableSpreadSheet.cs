...
namespace Simulator
{
    class SharableSpreadSheet
    {
        ...
        {
            BlockWriter = new Semaphore(1, 1);// only one Tread can accsses 
            //initial the counting semaphore depends on the nUsers.
            if (nUsers != -1)
                SearchingSemaphore = new Semaphore(1, nUsers);
            else
        ...
        public void exchangeRows(int row1, int row2)
        {
            if (row1 >= 0 && row1 < rows && row2 >= 0 && row2 < rows)
            {
                ...
                for (int j = 0; j < cols; j++)
                {
                    ...
                    SpreadSheetMatrix[row1, j] = SpreadSheetMatrix[row2, j];
                    SpreadSheetMatrix[row2, j] = temp;
                }
                //END OF CS
                BlockWriter.Release();
            }
        ...
        public void exchangeCols(int col1, int col2)
        {
            ...
            if (col1 >= 0 && col1 < cols && col2 >= 0 && col2 < cols)
            {
                ...
                for (int j = 0; j < rows; j++)
                {
                    ...
                    SpreadSheetMatrix[j, col1] = SpreadSheetMatrix[j, col2];
                    SpreadSheetMatrix[j, col2] = temp;
                }
                //END OF CS
                BlockWriter.Release();
            }
            ...
            if (row1 >= 0 && row1 < rows)
            {
                ...
                for (int i = 0; i <= rows; i++)
                {
                    for (int j = 0; j < cols; j++)
                    {
                        ...
                        if (i == row1+1)
                        {
                            newSpreadSheetMatrix[i, j] = "NewRow";
                            index = -1;
                        }
                        //else copy to the new SpreadSheetMatrix
                        else
                        {
                            newSpreadSheetMatrix[i, j] = SpreadSheetMatrix[index+i, j];
                        }
                    }
                }
            ...
            if (col1 >= 0 && col1 < cols)
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    for (int j = 0; j <= cols; j++)
                    {
                        ...
                        if (j == col1+1)
                        {
                            newSpreadSheetMatrix[i, j] = "NewCol";
                        }
                        //else copy to the new SpreadSheetMatrix
                        else
                        {
                            newSpreadSheetMatrix[i, j] = SpreadSheetMatrix[i, index];
                            index++;
                        }
        ...
        public void load(string fileName)
        {
            ...
            int coldFile = lines[0].Split(',').Length - 1;
